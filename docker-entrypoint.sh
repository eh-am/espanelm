...
if [[ "$size" -gt "0" ]]; then
	echo "Syncing files..."
	aws s3 cp --acl='public-read' news.json s3://espanelm-website/news.json

	aws s3 cp --acl='public-read' news.json s3://espanelm-website/news_"$(date '+%Y-%m-%d_%H-%M-%S_%s')".json
else
